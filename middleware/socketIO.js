...
const getNextMusicInQueue = async (roomId) => {
  try {
    ...
    // check if room exists and has music in queue
    if (room && room.musicInQueue && room.musicInQueue.length > 0) {
      // Get index of currentMusicPlay in musicInQueue
      const currentMusicPlayIndex = room.musicInQueue.indexOf(
        room.currentMusicPlay,
      );

      // Check if currentMusicPlay is the last music in queue or not exist
      if (
        currentMusicPlayIndex === -1 ||
        currentMusicPlayIndex === room.musicInQueue.length - 1
      ) {
        // Get the first music in queue
        const nextMusicId = room.musicInQueue[0];
        const nextMusic = await Music.findById(nextMusicId);
        return nextMusic;
      } else {
        // Get the next music in queue
        const nextMusicId = room.musicInQueue[currentMusicPlayIndex + 1];
        const nextMusic = await Music.findById(nextMusicId);
        return nextMusic;
      }
    }
    return null; // Return null if room not exist or has no music in queue
  } catch (error) {
  ...
  io.on('connection', (socket) => {
    ...
    const createTimer = async (id) => {
      ...
      const timer = setInterval(async () => {
        try {
          if (!roomTimers[id]) return;
          const roomSize = io.sockets.adapter.rooms.get('room' + id)?.size || 0;
          if (roomSize === 0) {
            console.log('clear');
            const roomNew = await Room.findOneAndUpdate(
              { _id: id },
              { $set: { currentTime: 0 } },
              { new: true },
            );
          ...
          if (
            roomCurrent.currentMusicPlay &&
            newTime >= roomCurrent.currentMusicPlay.duration
          ) {
            const music = await getNextMusicInQueue(roomCurrent._id);
            const roomNew = await Room.findOneAndUpdate(
              { _id: roomCurrent._id },
              { $set: { currentMusicPlay: music, currentTime: 0 } },
              { new: true },
            )
              .populate({
                path: 'musicInQueue',
            ...
            if (!roomTimers[roomNew._id]) return;
    ...
    socket.on(
      'join_room',
      asyncHandler(async (room) => {
        try {
          socket.join('room' + room._id);
          const roomSize = io.sockets.adapter.rooms.get('room' + room._id).size;
          console.log(
            `User with id: ${socket.id} joined room ${room._id} with ${roomSize} people`,
          );
      ...
      asyncHandler(async (room) => {
        try {
          if (socket.rooms.has('room' + room._id)) {
            ...
            const roomSize =
              io.sockets.adapter.rooms.get('room' + room._id)?.size || 0;
            console.log(
              `User with id: ${socket.id} leaved room ${room._id} with ${roomSize} people`,
            );
            socket
              .to('room' + room._id)
              .emit('update-people-in-room', roomSize);
    ...
    socket.on('disconnect', () => {
      try {
        ...
        rooms.forEach((room) => {
          ...
          console.log(
            `User with id: ${socket.id} left room ${room} with ${roomSize} people`,
          );
      ...
      try {
        if (!data.room.currentMusicPlay) {
          ...
          const newRoomUpdate = await Room.findOneAndUpdate(
            { _id: newRoom._id },
            { $set: { currentMusicPlay: newRoom.musicInQueue[0]._id } },
            { new: true },
          )
            .populate({
              path: 'musicInQueue',
          ...
          if (!roomTimers[data.room._id].timer)
            roomTimers[data.room._id].timer = await createTimer(
              newRoomUpdate._id,
            );
          io.to('room' + newRoomUpdate._id).emit(
            'update_music_current',
            newRoomUpdate,
          );
          io.to('room' + newRoom.roomId).emit(
            'receive_playlist_change_room',
            newRoomUpdate,
          );
        } else
          io.to('room' + data.room.roomId).emit(
            'receive_playlist_change_room',
            data.room,
          );
      } catch (e) {

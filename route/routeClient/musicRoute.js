...
const {
  ...
  musicMessageUpload,
  deleteMusicById,
} = require('../../controller/controllerClient/musicClientController');
const { authenticateToken } = require('../../authentication/jwtAuth');

router
  .route('/')
  .post(authenticateToken, uploadMusic)
  .get(authenticateToken, getMusicCurrentUser);
...
router
  .route('/update-music-privacy')
  .put(authenticateToken, updateMusicPrivacyStatus);
...
router
  .route('/:id')
  .get(getMusicByID)
  .put(authenticateToken, updateMusicInformation)

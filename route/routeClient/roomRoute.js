...
} = require('../../controller/controllerClient/roomClientController');

const { authenticateToken } = require('../../authentication/jwtAuth');

router
  .route('/')
  .get(authenticateToken, getCurrentRoomMusic)
  .post(authenticateToken, postNewRoom);

router
  .route('/post-music/:id')
  .put(authenticateToken, postNewMusicToRoomPlaylist);

router
  .route('/remove-music/:id')
  .put(authenticateToken, removeMusicToRoomPlaylist);

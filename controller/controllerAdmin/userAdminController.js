    ...
    if (req.isAuthenticated()) {
      try {
        ...
        const mapResult = result.map((user) => ({
          ...
          Email: user.email,
          Role: user.role, // Display genre as string
          Gender: user.gender,
...
const searchUser = asyncHandler(async (req, res) => {
  if (req.isAuthenticated()) {
    ...
    // Using regex to search for user
    try {
      const searchUserRegex = new RegExp(searchUser, 'i'); // i for case-insensitive
  ...
  try {
    if (req.isAuthenticated) {
      try {
        ...
        return res.status(200).json({ message: 'success', data: mapResult });
      } catch (error) {
        console.log(error);
        res.sendStatus(500);
      }

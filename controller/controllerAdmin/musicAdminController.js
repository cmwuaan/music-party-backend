    ...
    if (req.isAuthenticated()) {
      try {
        const result = await Music.find({
          ...
          .select('musicName author genre createdAt'); // Select the fields you want to display

        const resultWithMessageCount = result.map((music) => ({
          id: music._id,
          Pending: music.musicName,
          Artist: music.author,
          Genre: music.genre.join(', '), // Display genre as a string
        ...
        return res
          .status(200)
          .json({ message: 'Update success', data: resultWithMessageCount });
      } catch (e) {
...
const searchUnauthenticatedMusic = asyncHandler(async (req, res) => {
  if (req.isAuthenticated()) {
    ...
    // Use regular expressions to create search conditions
    try {
      const searchMusicRegex = new RegExp(searchMusic, 'i'); // i is case-insensitive
      const music = await Music.find({
        ...
        .skip(index) // Skip the first records to index
        .limit(quantity)
        .select('musicName author genre createdAt'); // Select the fields you want to display
      ...
      res
        .status(200)
        .json({ message: 'Success', data: resultWithMessageCount });
    } catch (e) {
      console.log(e);
...
const approveSong = asyncHandler(async (req, res) => {
  if (req.isAuthenticated()) {
    try {
      ...
      const newMusic = await Music.updateOne(
        { _id: id },
        {
          $set: {
            isRequest: false,
...
const approveList = asyncHandler(async (req, res) => {
  if (req.isAuthenticated()) {
    try {
      ...
      const newListMusic = await Music.updateMany(
        { _id: { $in: idsToUpdate } },
        { $set: { musicAuthorize: approveStatus, isRequest: false } },
      );
  ...
  if (req.isAuthenticated()) {
    ...
    try {
      ...
      const music = await Music.find({
        ...
        .limit(quantity)
        .select('musicName author genre musicPrivacyType createdAt'); // Select the fields you want to display
      const resultWithMessageCount = music.map((music) => ({
        id: music._id,
        Song: music.musicName,
        Artist: music.author,
            ...
            try {
              ...
              const listMusic = await Music.find({})
                ...
                .populate({
                  ...
                  model: 'User',
                  select: ['avatar', 'displayName'], // Select the fields you want to display, e.g. 'avatar'
                });

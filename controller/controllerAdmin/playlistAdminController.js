...
const searchPlaylist = asyncHandler(async (req, res) => {
  if (req.isAuthenticated()) {
    ...
    // Using regex to search for playlist
    try {
      const searchPlaylistRegex = new RegExp(searchPlaylist, 'i'); // i for case-insensitive
      const result = await Playlist.find({
        $or: [{ playlistName: { $regex: searchPlaylistRegex } }],
      })
        .populate('ownerPlaylistID', 'displayName')
        .sort({ playlistName: 1 })
  ...
  if (req.isAuthenticated()) {
    try {
      ...
      const { playlistName, privacyStatus, description, avatarPlaylist } =
        req.body;
      const updateData = {
        playlistName: playlistName ? playlistName : prevPlaylist.playlistName,

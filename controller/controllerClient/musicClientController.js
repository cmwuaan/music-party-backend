  ...
  try {
    const music = await Music.findById(req.params.id).populate({
      ...
      populate: {
        path: 'userId',
        model: 'User',
        select: ['avatar', 'displayName'], // Display only avatar and displayName
      },
    });
    if (music) {
...
const findMusicByNamePublic = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const music = await Music.find({
      ...
      .sort({ view: desc }) // Sort by view
      .skip(index) // Skip the first {index} documents
      .limit(quantity); // Limit the number of documents
    res.status(200).json({ message: 'Success', data: music });
  } catch (e) {
    console.log(e);
...
const getTopMusic = asyncHandler(async (req, res) => {
  // Get value from query parameter 'index'
  // If not found, set default value to 0
  try {
    const quantity = 20;
    ...
    if (req.isAuthenticated()) {
      try {
        ...
        for (let i = 0; i < genre.length; i++) {
          try {
            const existingGenre = await MusicGenre.findOne({
              musicGenre: genre[i],
            });

            if (existingGenre) {
              await MusicGenre.updateOne(
                { _id: existingGenre._id },
                { $inc: { musicQuantity: 1 } },
              );
            } else {
        ...
        const music = await Music.create({
          ...
          isRequest:
            musicPrivacyType === MusicTable.MUSIC_PRIVACY_PUBLIC ? true : false,
        });
        res.status(200).json({
          message: 'Success',
          data: music,
        ...
        if (existedMusic.musicPostOwnerID !== req.user.user._id) {
          try {
            ...
            const musicUpdate = {
              ...
              isRequest: musicPrivacyType
                ? musicPrivacyType === MusicTable.MUSIC_PRIVACY_PUBLIC &&
                  existedMusic.musicAuthorize ===
                    MusicTable.MUSIC_AUTHENTICATION_UNAUTHORIZE
                  ? true
                  : false
                : existedMusic.isRequest,
    ...
    if (req.isAuthenticated()) {
      try {
        ...
        const result = await Music.updateOne(
          { _id: _id },
          { $set: { musicPrivacyType: musicPrivacyType } },
        );
        res.status(200).json({
...
const listenMusic = asyncHandler(async (req, res) => {
  try {
    ...
    } else res.status(404).json({ message: 'Music not existed', data: null });
  } catch (e) {
    return res.status(500).json({ message: 'Server error' });
  }
});
...
const musicMessageUpload = asyncHandler(async (req, res) => {
  try {
    ...
    } else return res.status(401).json({ message: 'Unauthorize' });
  } catch (e) {
    return res.status(500).json({ message: 'Server error' });
  }
});

  ...
  try {
    if (req.isAuthenticated()) {
      try {
        ...
        const newPlaylist = await Playlist.findById(playlist._id).populate({
...
const getPlaylistByID = asyncHandler(async (req, res) => {
  try {
    const playlistPublic = await Playlist.findOne({
      _id: req.params.id,
    ...
    if (!playlistPublic) {
      if (req.isAuthenticated()) {
        try {
          const playlistPrivate = await Playlist.findOne({
            _id: req.params.id,
            ownerPlaylistID: req.user.user._id,
          })
            .populate({

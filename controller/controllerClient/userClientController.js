...
const getUserByID = asyncHandler(async (req, res) => {
  try {
    ...
    else res.status(404).json({ message: 'User not existed', data: null });
  } catch (ex) {
...
const updateUserById = asyncHandler(async (req, res) => {
  try {
    if (req.isAuthenticated()) {
      ...
      } else res.status(401).json({ message: 'Unauthorize role' });
    } else {
      res.status(401).json({ message: 'Unauthorize' });
    }
  } catch (e) {
...
const createNewAccount = asyncHandler(async (req, res) => {
  try {
    ...
    if (user !== null) {
      res
        .status(409)
        .json({ isSuccess: false, message: 'Account already existed' });
    } else {
...
const createAdminAccount = asyncHandler(async (req, res) => {
  if (req.isAuthenticated() && req.user.user.role === UserTable.ROLE_ADMIN) {
    try {
      const { username, email, avatar, gender, password, displayName, role } =
        req.body;
  ...
  if (req.isAuthenticated() && req.user.user.role === UserTable.ROLE_ADMIN) {
    ...
    try {
      const user = await User.find({})
        .sort({ createdAt: desc }) // Arrange by the field 'createdAt' (decreasing)
        .skip(index) // Skip the first {index} documents
        .limit(quantity); // Limit the result returned to 'quantity'
      res.status(200).json({ message: 'Success', data: user });
    } catch (e) {
      res.status(500).json({ message: 'Server error' });
    }

...
const getRoomByID = asyncHandler(async (req, res) => {
  try {
    if (req.isAuthenticated()) {
      const room = await Room.findById(req.params.id)
        .populate({
          ...
          populate: {
            path: 'userId',
            model: 'User',
            select: ['_id', 'avatar', 'displayName'], // Select the fields you want to display, for example 'avatar'
          },
        })
        .populate({
          path: 'musicInQueue',
        ...
        .populate({
          ...
          populate: {
            path: 'userId',
            model: 'User',
            select: ['avatar', 'displayName'], // Select the fields you want to display, for example 'avatar'
          },
        })
        .populate({
      ...
      if (room) res.status(200).json({ message: 'Success', data: room });
...
const getCurrentRoomMusic = asyncHandler(async (req, res) => {
  try {
    if (req.isAuthenticated()) {
      ...
      const room = await Room.find({ roomOwner: ownerId }).populate({
        path: 'musicInQueue',
        select: ['author', 'musicName', '_id', 'url', 'imgUrl', 'duration'],
      });
